...
private:
	...
	VkImage colorMSAAImage;
	VkDeviceMemory colorMSAAImageMemory;
	VkImageView colorMSAAImageView;
	...
	bool isDeviceSuitable(VkPhysicalDevice device)
	{
		...
		VkFormatProperties prop[2];
		
		vkGetPhysicalDeviceFormatProperties(device, VK_FORMAT_R8G8B8A8_UNORM, &prop[0]);
		vkGetPhysicalDeviceFormatProperties(device, VK_FORMAT_D32_SFLOAT_S8_UINT, &prop[1]);


		VkImageFormatProperties propImg[2];

		vkGetPhysicalDeviceImageFormatProperties(device, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_TYPE_2D, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT, 0, &propImg[0]);
		vkGetPhysicalDeviceImageFormatProperties(device, VK_FORMAT_D32_SFLOAT_S8_UINT, VK_IMAGE_TYPE_2D, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, 0, &propImg[1]);
		...
		msaaSamples = t == 1 ? static_cast<VkSampleCountFlagBits>((uint32_t)std::min(std::pow(2, (uint32_t)std::log2(counts)), (double)VK_SAMPLE_COUNT_64_BIT)) : VK_SAMPLE_COUNT_1_BIT;
	...
	{
		VkFormat colorMSAAFormat = swapChainImageFormat;

		createImage(swapChainExtent.width, swapChainExtent.height, 1, msaaSamples, colorMSAAFormat, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT | VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, colorMSAAImage, colorMSAAImageMemory);
		colorMSAAImageView = createImageView(colorMSAAImage, colorMSAAFormat, VK_IMAGE_ASPECT_COLOR_BIT, 1);

		transitionImageLayout(colorMSAAImage, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL,
	...
	void createImage(uint32_t width, uint32_t height, int32_t mipLevels, VkSampleCountFlagBits numSamples, VkFormat format, VkImageTiling tiling,
	...
	VkImageView createImageView(VkImage image, VkFormat format, VkImageAspectFlags aspectFlags, int32_t mipLevels)
	{
	...
	void transitionImageLayout(VkImage image,
		...
		VkImageAspectFlags imageAspectFlags, uint32_t mipLevels)
	{
		VkCommandBuffer commandBuffer = beginSingleTimeCommands();
	...
	void cleanupSwapChain()
	{
		...
		vkDestroyImageView(device, colorMSAAImageView, nullptr);
		vkDestroyImage(device, colorMSAAImage, nullptr);
		vkFreeMemory(device, colorMSAAImageMemory, nullptr);
